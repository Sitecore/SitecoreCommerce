...
namespace Plugin.Sample.Promotions.Generator
{
    ...
    ///     </cref>
    /// </seealso>
    [PipelineDisplayName("EvaluatePromosFilterByBenefitType")]
	public class EvaluatePromosFilterByBenefitType : PipelineBlock<IEnumerable<Promotion>, IEnumerable<Promotion>, CommercePipelineExecutionContext>
	{
        /// <summary>
        /// The execute.
        /// </summary>
        ...
        public override Task<IEnumerable<Promotion>> Run(IEnumerable<Promotion> arg, CommercePipelineExecutionContext context)
		{
            ...
            var discoverPromotionsArgument = context.CommerceContext.GetObjects<DiscoverPromotionsArgument>().FirstOrDefault();

            var benefitType = discoverPromotionsArgument?.BenefitTypeFilter;

            if (cart != null)
            {
                cart.GetComponent<MessagesComponent>().AddMessage(
                                context.GetPolicy<KnownMessageCodePolicy>().Promotions,
                                $"EvaluatePromosFilterByBenefitType.{benefitType.Name}.PromotionsRemaining: {arg.Count()}");
            }
