                ...
                foreach(var itemVariationComponent in sellableItem.GetComponent<ItemVariationsComponent>().GetComponents<ItemVariationComponent>())
                {
                    if (itemVariationComponent.HasComponent<InventoryComponent>())
                    {
                        ...
                        foreach(var inventoryAssociationTarget in inventoryComponent.InventoryAssociations)
                        {
                            ...
                            //var inventoryItem = await this._commerceCommander.Command<GetInventoryInformationCommand>().Process(context.CommerceContext, inventoryAssociationTarget.InventorySet.EntityTarget, sellableItem.Id, "", false);
                            //entityView.Properties.Add(
                            //    new ViewProperty
                            //    {
                            //        Name = $"AllocationFrom-{inventorySlotKey}",
                            //        DisplayName = $"Allocation From {inventoryAssociationTarget.InventorySet.EntityTarget.Replace("Entity-InventorySet-", "")} ({inventoryItem.Quantity})",
                            //        IsHidden = false,
                            //        //IsReadOnly = true,
                            //        IsRequired = true,
                            //        RawValue = inventoryItem.Quantity
                            //    });
                        }
                    }
                }
